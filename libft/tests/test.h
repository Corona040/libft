...
/*   Updated: 2024/10/25 20:29:26 by ecorona-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
# define _ft_announce() do {char str[256]; memset(str, 0, 256); sprintf(str, DFLT"%-15s %d - ", __func__, __LINE__); write(STDOUT_FILENO, str, strlen(str));} while (0)

# define _ft_FAIL(severity) printf(severity"%-7s"DFLT"\n", "KO")
# define _ft_PASS() printf(GREEN"%-7s"DFLT"\n", "OK")
